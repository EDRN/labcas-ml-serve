...
def init_get_environment(environment_name, environments_config):
    ...
    ray.serve.start(
        detached=True,
        http_options={
            "port": environment_info["serve_port"],
            "host": "0.0.0.0",
            "middlewares": [Middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"])],
        },
    )
...
def deploy(environments_config, deployment_config):
    ...
    for deployment_info in deployment_config["deployments"]:
        ray_deployment = serve.deployment(deployment_info["class"]).options(
            ...
            _autoscaling_config={
                "target_num_ongoing_requests_per_replica": deployment_info["target_num_ongoing_requests_per_replica"],
...
def create_environments(environments_config, head=False):
    for environment_name, environment_info in environments_config.items():
        command = (
            ". "
            + os.path.join(environment_info["pyenv"], "activate")
            + " && ray start"
            + " --port "
            + environment_info["port"]
            + " --object-store-memory "
            + environment_info["object_store_memory"]
            + " --dashboard-host 0.0.0.0"
            + " --num-cpus "
            + environment_info["num_cpus"]
            + (" --head" if head else "")
...
def kill_environments(environments_config):
    for environment_name, environment_info in environments_config.items():
        command = ". " + os.path.join(environment_info["pyenv"], "activate") + " && ray stop"
        print("RUNNING on shell:", command)
        os.system(command)
